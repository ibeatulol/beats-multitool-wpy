...
def run_gui_py_and_exit():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    py_path = os.path.join(script_dir, "gui.py")
    if not os.path.isfile(py_path):
        print(RED + f"gui.py not found at {py_path}. Exiting instead." + RESET)
        sys.exit(0)
    try:
        if os.name == "nt":
            subprocess.Popen(["python", py_path], shell=False)
        else:
            subprocess.Popen(["python3", py_path], shell=True)
    except Exception as e:
        print(RED + f"Failed to launch gui.py: {e}" + RESET)
    sys.exit(0)
...
def main():
    while True:
        single_scan_flow()
        # Only two choices: scan again (01) or exit and run gui.py (02)
        while True:
            print()
            print("Options:")
            print("  01) Scan again")
            print("  02) Exit and run gui.py")
